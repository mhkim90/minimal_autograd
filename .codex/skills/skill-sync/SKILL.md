...
For a pure target sync of an already merged source revision, record that source
revision and skip source implementation; do not skip target preflight or gates.
...
5. Create or update one **draft** PR per target through GitHub MCP. Include the
   source revision, manifest, validation results, and any approved variant.
