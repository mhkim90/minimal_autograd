...
- **phase-gated-implementation**: When implementing multi-phase work from a plan, PR description, issue, design doc, or approved checklist. From Codex, Codex/Sol owns the final gate, OpenCode `agent="luna"` handles normal role-bound implementation, the selector-free configured default is reserved for explicit mechanical/economy work, `agent="terra"` performs focused read-only review, and Claude is a scarce independent read-only reviewer. From Claude Code, Claude orchestrates and controls publication, Luna implements, the configured default handles explicit mechanical/economy work, Terra reviews, and Codex/Sol supplies the independent high-risk read-only gate. Fail closed when named-agent selection cannot be proved. Use risk and evidence, not fixed token percentages.
