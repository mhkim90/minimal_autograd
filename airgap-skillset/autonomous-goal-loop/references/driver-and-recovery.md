...
`reference-impl/loop/run.sh` is the working lease-and-sentinel entry
...
`reference-impl/loop/negative_test.py` is the working probe set.
...
in-process process-group kill is best effort. A controller receipt proving the
cgroup backstop kills a `setsid()` descendant that ignores SIGTERM proves only
that, not in-process ordering; a separate receipt is needed to discriminate
approved-unit acceptance from a missing `SYSTEMD_UNIT` or an outside-unit
control. This package does not ship a pre-recorded receipt — one machine's
receipt is not evidence for another (see `PORTING.md`) — regenerate both via
`make_containment_receipt.py` before relying on them; see
`reference-impl/README.md`. The collector and driver add no uncharged timeout
grace period.
...
one, through whichever single-model harness is active locally (Claude Code,
OpenCode, or Copilot — this package assumes exactly one backend model
reachable through it, same as `phase-gated-implementation` on this branch).
Reuse that harness's own headless/print-mode invocation shape (for Claude
Code, `claude -p "$PROMPT" --model <charter model> --output-format json
--max-turns <cap> --permission-mode acceptEdits --add-dir <worktree>
--settings <worktree>/.claude/settings.json` is the working shape); the
requirements below apply regardless of which harness supplies it.
