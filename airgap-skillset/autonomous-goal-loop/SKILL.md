...
The working implementation in `reference-impl/loop/` is the normative
...
| Implementing an evidence-class component | `reference-impl/loop/` |
| Code changes made in-loop | `karpathy-best-practices` |
| Owner wants the branch published | `phase-gated-implementation` |
| Resume or cross-repository work | `handoff` + `phase-gated-implementation`'s "Local continuity is discovery-only" invariant |

This variant assumes exactly one backend model reachable through the local
harness, same as `phase-gated-implementation` on this branch. There is no
multi-model routing: the open-network `opencode-delegate` named-agent
selection does not apply here and is intentionally absent from this
package. Where the open-network version would delegate a ratchet/search
implementation step, implement it directly under the ceremony tiers
instead — there is nothing to route to when every role resolves to the
same weights. `Resume or cross-repository work` above is about the
*controlling agent's own session* resuming, not the loop's execution
resume — the loop's own Resume procedure
([`references/charter-and-ledger.md`](references/charter-and-ledger.md))
is disk-only and self-contained regardless of any external memory skill.
...
`phase-gated-implementation`'s decision, never pre-authorized by a run. On
this branch that means a ticket-ID-first commit and a markdown delivery
draft in the untracked `delivery/` folder, never a real PR — unchanged from
how any other completed phase enters that skill's gates.
