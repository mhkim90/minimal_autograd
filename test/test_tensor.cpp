...
void test_predicate_selection_and_status_cpu() {
    const std::vector<float> lhs_values{-1.f, 0.f, 2.f,
                                        std::numeric_limits<float>::quiet_NaN()};
    const std::vector<float> rhs_values{-1.f, 1.f, 1.f, 4.f};
    Variable lhs(Tensor::from_host(lhs_values.data(), Shape{4}), true);
    Variable rhs(Tensor::from_host(rhs_values.data(), Shape{4}), true);
    ...
    CHECK(!mask.requires_grad());
    CHECK_THROWS_AS(std::invalid_argument,
                    ag::less_equal(lhs, Variable(Tensor::zeros(Shape{3}))));

    const std::vector<float> true_values{10.f, 20.f, 30.f, 40.f};
    const std::vector<float> false_values{-1.f, -2.f, -3.f, -4.f};
    Variable condition(Tensor::from_host(mask_values.data(), Shape{4}), true);
    Variable when_true(Tensor::from_host(true_values.data(), Shape{4}), true);
    Variable when_false(Tensor::from_host(false_values.data(), Shape{4}), true);
    CHECK_THROWS_AS(std::invalid_argument,
                    ag::where(condition, when_true,
                              Variable(Tensor::zeros(Shape{3}))));
    Variable selected = ag::where(condition, when_true, when_false);
    selected.backward(Tensor::ones(Shape{4}));
    ...
    const std::vector<float> finite_values{1.f, -2.f, 0.f};
    Tensor finite = Tensor::from_host(finite_values.data(), Shape{3});
    CHECK(!ag::all_true(finite));
    CHECK(ag::all_finite(finite));
    const float nonfinite_values[] = {1.f, std::numeric_limits<float>::infinity(),
                                      std::numeric_limits<float>::quiet_NaN()};
    Tensor nonfinite = Tensor::from_host(nonfinite_values, Shape{3});
    CHECK(!ag::all_true(nonfinite));
    CHECK(!ag::all_finite(nonfinite));
    CHECK(ag::all_true(Tensor::from_host(
        std::vector<float>{1.f, -2.f}.data(), Shape{2})));
    CHECK(ag::all_true(Tensor::empty(Shape{0, 2})));
    CHECK(ag::all_finite(Tensor::empty(Shape{0, 2})));
    report("Tensor ops: CPU less_equal/where VJPs and status predicates");
}

void test_fixed_grid_support_cpu() {
    ...
    Variable offset(Tensor::from_host(offsets.data(), Shape{9}));
    Variable weight(Tensor::from_host(weights.data(), Shape{9}));
    Variable zero(Tensor::zeros(Shape{9}));
    ...
    report("Tensor ops: fixed-grid support matches clamped floor boundaries");
}
...
int main() {
    ...
    test_predicate_selection_and_status_cpu();
    test_fixed_grid_support_cpu();
